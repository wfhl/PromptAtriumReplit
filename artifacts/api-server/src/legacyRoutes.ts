...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/objects/serve/*servePath", async (req, res) => {
    try {
      const objectPath = decodeURIComponent(req.path.replace("/api/objects/serve", "") || "/");
  ...
  app.get('/api/objects/serve/*servePath', async (req: any, res) => {
    try {
      const path = decodeURIComponent(req.path.replace('/api/objects/serve', '') || '/');
